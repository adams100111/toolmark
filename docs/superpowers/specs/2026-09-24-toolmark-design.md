...
| **M2 execution rulings (2026-09-25)**: array ops only on declared arrays (`$`-keyed data on records stays data); duplicate fill paths → `invalid`; per-fill node budget 10000 ("Input too complex"); `fromJsonSchema` rejects unsafe `pattern`s at construction (nested quantifiers, overlapping quantified alternation, backreferences), never pattern-tests values > 10000 chars, validates `additionalProperties` schemas and stops at 100000 evaluations; options 10 s / 50 items / `{value,title}`; files: `maxFiles` default 10 (≤ 100), ref ≤ 2048, url ≤ 8192, ≤ 100 file refs per fill (`MAX_FILE_REFS_PER_FILL`, fixed), exact-origin `allowOrigins` without `*`, `http:` only for loopback, `{ref}` resolvers bounded by the call signal; wizard: all-or-nothing commits with rollback, submit summary and app `submit(data)` see the mounted step's live values, stepwise `refresh()` retries; DOM: read-only controls never written, password/one-time-code exclusion sticky, labels/controls in editable or ignored regions dropped, buttons need `data-tool-description`, submit buttons ≥ consequential with the form's stale snapshot, table caps 32 columns / 200000 chars, description caps 500/200, rescans skip ignored regions, `FormToolOptions.{origin,nativeName,hints}` internal (submit confirmation can't be removed except by `toolautosubmit`); Inertia: non-GET props tools need closed root schemas, `_method`/`_token` refused, canonical same-origin URLs, caps (description 2048, title 128, 64 entries, schema 32 KiB), `<Form>` correlates visits by url+method and settles `error` if no visit starts within 1000 ms (Inertia 3 error events carry no visit — known limit), `navigationTool` GET-only, returns the canonical href, requires ≥ 1 route; `emitEvent`/`isDevRegistry` are `@internal` core exports; React hooks use the registry's `dev` flag. | M2 execution ledger |
| **M3 execution rulings (2026-09-25)**: bridge `caller` is `'inapp' \| 'mcp'` (runtime-validated); each tool's `state()` owns redaction (`'[redacted]'`), element-based sensitivity is sticky per adapter and per wizard step, `[]` wildcard sensitive paths expand (≤ 10000 values, else the whole array is redacted); OTel records no payloads by default, error spans carry only `Tool failed`, and when opted in redacts input at the tool's real shape (`values.<p>`, `steps.<step>.<p>`); WebMCP: app-supplied polyfill loader, `toolchange` resync, stale handles refused, execute input bounded (1 MiB, depth 64); MCP: SDK low-level `Server` with `serveStdio(…, { legacy: 'serve' })`, conformance tool deferred (HTTP-only), mapping caps (description 2048, title 256, schema 32 KiB / depth 32, list 200 tools / 256 KiB), untrusted marking via description suffix + text prefix + `_meta['toolmark/untrustedContent']`; pairing: 127.0.0.1 bind, exact `Host`/`Origin`, 40-bit single-use code, 256-bit token, 250 ms refusal after `4401`, 3000 ms first frame, 1 KiB handshake frames, `--call-timeout` 600000 with `cancel`, cancels for calls ended while detached flushed on same-`clientId` resume, terminal closes (`4400/4401/4408/4409`) and `1001` withdraw in-flight confirmations (1001 keeps the session), local processes out of scope (§14); `useToolAnchor` keeps callback identity per registry (React 18 StrictMode); milestone version bumps are redone at integration after the previous milestone merges. | M3 execution ledger |
| **M4 execution rulings (2026-09-25)**: tour `confirming` is driven by core confirm events (any pending confirmation from any tool or caller releases the `do`-mode trap, backdrop and stacking), `TourState.busy` flags an in-flight `do` call (Next/Back `aria-disabled`), `do` steps run once on entry and `back()` never auto-runs an unrun step, guide matching uses prefix paths; `toolmark` bin is a dedicated `bin.ts` entry (works through npm/npx symlinks) and accepts `toolmark lint …`, validates both `--manifest` and `--url` manifests against the manifest schema, strips control characters from output, exits 2 with usage when given no input, compiles each tool schema with a fresh Ajv; the TypeSafe judge sends names/titles/descriptions/schemas plus the page origin+path only, is bounded to 120 s per page with at most one retry and reports `judge/timeout`; Laravel example: PHP via Docker when absent (`scripts/php.sh`), `// file:` markers on line 2, the reference check runs in both directions, page-supplied `id`/`confirmId`/`clientId` match `^[A-Za-z0-9_-]{1,128}\z`, demo seeding and test routes only in `local`/`testing`, e2e bundle in `public/build-e2e`; docs deploy stays disabled while the repo is private; the `mcp` CLI installs signal handlers before startup work. | M4 execution ledger |
| **M5 execution rulings (2026-09-25)**: the 1.0 security review (`docs/security/review-2026.md`, SEC-1…SEC-30, all resolved with regression tests) is binding: `jsonSchema`-only tools validate against it (keywords outside the `fromJsonSchema` subset are ignored, documented); `fill` never writes undeclared keys; confirmation expiry is checked at approval; `callTimeoutMs` (default 120000) bounds signal-less runs, `confirmPending` and undo, and pauses while an inline `ctx.confirm` is open; confirmation payloads, OTel changes and form results redact sensitive paths including `[]` array paths, and redact everything when `sensitivePaths()` throws; an edited approval restores stored secrets only onto the same row, otherwise refuses `invalid` "Re-enter sensitive field", and never lets a `'[redacted]'` placeholder reach a tool; fills are `untrustedContent`. Release: `release.yml` publishes per tarball with `npm publish --provenance --tag latest` (not `changesets publish`, whose pnpm path drops provenance) only from `main`, non-PR, public repo, `TOOLMARK_PUBLISH_ENABLED` + owner-approved `npm-release`; caches are off in every job feeding publish; tarballs are checked against the plan (manifest, duplicate/PAX entries, `npm publish --dry-run`) and tags must point at the release SHA; `release-dry-run` runs the whole path on every PR (synthetic plan once 1.0.0 is on npm) and is the only required `release.yml` check; `check-workflows` enforces read-only, secret-free `pull_request` jobs. Size budgets: `@toolmark/testing` 17.5 kB and `/vitest` 17 kB (they bundle core), `@toolmark/core/otel` 2.75 kB. The owner steps run through `scripts/owner-publish-wizard.sh`. | M5 execution ledger |
